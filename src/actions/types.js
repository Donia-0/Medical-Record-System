export const GET_ERRORS = "GET_ERRORS";
export const SET_CURRENT_USER = "SET_CURRENT_USER";
export const GET_PROFILE = "GET_PROFILE";
export const LOG_OUT = "LOG_OUT";
export const PROFILE_LOADING = "PROFILE_LOADING";
export const PROFILE_NOT_FOUND = "PROFILE_NOT_FOUND";
export const CLEAR_CURRENT_PROFILE = "CLEAR_CURRENT_PROFILE";
export const GET_BLOODPRESSURE = "GET_BLOODPRESSURE";
export const LOADING = "LOADING";
export const ADD_EXAMINATION = "ADD_EXAMINATION";
export const EDIT_EXAMINATION = "EDIT_EXAMINATION";
export const GET_GLUCOSE = "GET_GLUCOSE";
export const GET_SURGERY = "GET_SURGERY";
export const GET_EXAMINATION = "GET_EXAMINATION";
export const GET_DETAIL_BLOODPRESSURE = "GET_DETAIL_BLOODPRESSURE";
export const SEARCH_PATIENT = "SEARCH_PATIENT";
export const CLEAR_PATIENT_PROFILE = "CLEAR_PATIENT_PROFILE";
export const GET_ALLERGIES = "GET_ALLERGIES";
export const GET_DETAIL_ALLERGY = "GET_DETAIL_ALLERGY";
export const CLEAR_RECORDS = "CLEAR_RECORDS";
export const GET_DETAIL_SURGERY = "GET_DETAIL_SURGERY";
export const PATIENT_RECORD = "PATIENT_RECORD";
export const NOT_FOUND = "NOT_FOUND";
export const GET_LABTEST = "GET_LABTEST";
export const GET_PRESCRIPTION = "GET_PRESCRIPTION";
export const GET_PRESCRIPTION_FOR_EACH_EXAMINATION =
  "GET_PRESCRIPTION_FOR_EACH_EXAMINATION";
export const GET_DETAIL_PRESCRIPTION = "GET_DETAIL_PRESCRIPTION";
export const GET_DETAIL_EXAMINATION = "GET_DETAIL_EXAMINATION";
export const GET_DETAIL_GLUCOSE = "GET_DETAIL_GLUCOSE";
export const GET_DETAIL_USER_REQUEST = "GET_DETAIL_USER_REQUEST";
export const GET_ALL_REQUESTS = "GET_ALL_REQUESTS";
export const ACCEPT_USER_REQUEST = "ACCEPT_USER_REQUEST";
